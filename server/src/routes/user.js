...
router.put('/users/:id', (req, res) => {
    const {id} = req.params
    const {nombres, apellidos, docuemento, telefono, email, clave, rol} = req.body
    userSchema.updateOne({_id: id}, {$set: {nombres, apellidos, docuemento, telefono, email, clave, rol}}).then((data) => res.json(data)).catch((err) => res.json({message: err}))
})
